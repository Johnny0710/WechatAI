...
from utils import weather,dispose

# 初始化管理员,这个名称为登录的微信好友列表中的名称或备注名
admin_name = 'admin_name'

# 初始化机器人,登录微信
bot = Bot(cache_path=True, console_qr=2, qr_path='otherMessage/qr_code.png')
...
def friend_msg(msg):
    ...
    if no_forward and msg.sender.name != admin.name:
    ...
    # 判断接收的消息类型是否为文本型,文本型为1 且不是群内的消息时
    if msg_type == 1 and not msg.member :
        ...
        # 当以上条件都不符合且不是管理员消息时,将文本消息发送给管理员
        # 并告知发信人
        if msg.sender.name != admin.name:
            admin.send_msg('有新消息\n{}'.format(msg))
            return '已将消息转发給小主'
    ...
    if msg_type > 1 and msg.sender.name != admin.name:
